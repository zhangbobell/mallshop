		...
		<ul>
			...
			<?php if($promo == 'time'){?>
				...
				<li class="current">
					...
					<?php foreach(Api::run('getPromotionListByCondition',array('#id#',$id)) as $key => $item){?>
			...
			<?php if($promo == 'groupon'){?>
				...
				<li class="current">
					...
					<?php foreach(Api::run('getRegimentListByGoodsid',array('#id#',$id)) as $key => $item){?>
					<?php $free_time = ITime::getDiffSec($item['end_time']);?>
					...
					<?php foreach(Api::run('getRegimentUserRelationTotalNum',array('#rid#',$item['rid'])) as $key => $value){?>
