...
class CountSum
{
    ...
    public function direct_count($id,$type,$buy_num = 1,$promo='',$active_id='')
    {
		...
		if($type == 'product')
		{
			...
			$productRow = $model->getObj('pro.id = '.$id.' and pro.goods_id = go.id and go.is_del = 0','pro.sell_price,pro.weight,pro.id as product_id,pro.spec_array,pro.goods_id,pro.store_nums,go.name,go.point,go.exp,go.img');
		...
		else
		{
			...
			$goodsRow   = $model->getObj('id = '.$id.' and is_del = 0','go.name,go.id as goods_id,go.img,go.sell_price,go.point,go.weight,go.store_nums,go.exp,0 as product_id');
